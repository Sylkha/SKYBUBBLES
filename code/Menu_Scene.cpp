...
namespace example
{
    ...
    void Menu_Scene::render (Graphics_Context::Accessor & context)
    {
        if (!suspended)
        {
            ...
            background = Texture_2D::create (ID(background), context, "backgroundMenu.png");
    ...
    void Menu_Scene::configure_options ()
    {
        ...
        float option_top = canvas_height / 2.5f + menu_height / 2.f;

        // Se establece la posición del borde superior de cada opción:

        for (unsigned index = 0; index < number_of_options; ++index)
        {
            options[index].position = Point2f{ canvas_width / 1.5f, option_top };
